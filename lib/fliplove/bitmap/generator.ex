defmodule Fliplove.Bitmap.Generator do
  ...
  def random_perlin_noise(width, height) do
    perlin_noise(width, height,
      # 0.05 to 0.3
      scale: :rand.uniform() * 0.25 + 0.05,
      # 1 to 3
      octaves: :rand.uniform(3),
      # 0.3 to 0.7
      persistence: :rand.uniform() * 0.4 + 0.3,
      # 0.4 to 0.6 (denser around 0.5)
      threshold: :rand.uniform() * 0.2 + 0.4
    )
  end
  ...
  def flow_field(width, height, opts \\ []) do
    ...
    field_fn = fn x, y ->
      ...
      {dx, dy} =
        Enum.reduce(field_components, {0.0, 0.0}, fn component, {acc_x, acc_y} ->
          # Calculate angle from this component
          angle =
            :math.sin((x + component.offset_x) * component.freq_x + component.phase) +
  ...
  defp trace_particle(matrix, field_fn, {start_x, start_y}, width, height, steps) do
    ...
    # Extract the final matrix from the tuple
    |> elem(2)
  end
    ...
    cond do
      ...
      :rand.uniform() < opts[:split_probability] ->
        ...
        if width > height do
          ...
          # Results in 0.3 to 0.7
          split_point = :rand.uniform() * 0.4 + 0.3
  ...
  defp in_mandelbrot_set?({c_real, c_imag}, max_iter) do
    ...
    result =
      Enum.reduce_while(1..max_iter, {0.0, 0.0}, fn _i, {z_real, z_imag} ->
        # Calculate z = z^2 + c
