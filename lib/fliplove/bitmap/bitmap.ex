defmodule Fliplove.Bitmap do
  ...
  defmacro defbitmap(opts \\ [], do: block) do
    ...
    quote do
      ...
      case bitmap do
        %Fliplove.Bitmap{} -> bitmap
        map when is_map(map) -> struct(Fliplove.Bitmap, Map.merge(%Fliplove.Bitmap{}, map))
      end
    end
  end

  defimpl Inspect, for: Fliplove.Bitmap do
  ...
  def bbox(bitmap) do
    ...
    if Enum.empty?(pixels) do
      # Return zero bounds for empty bitmap
      {0, 0, 0, 0}
    else
      ...
      for {line, y} <- Enum.with_index(Enum.reverse(lines)),
        ...
        value =
          cond do
            ...
            true ->
              raise "Invalid pixel character #{inspect(<<pixel>>)} (expected one of #{inspect(Enum.map(on, &<<&1>>))} for on, one of #{inspect(Enum.map(off, &<<&1>>))} for off)"
          end
  ...
  def merge(bitmap1, bitmap2, options \\ []) do
    ...
    cond do
      Enum.empty?(bitmap1.matrix) ->
        bitmap2

      Enum.empty?(bitmap2.matrix) ->
        bitmap1
