defmodule Flipdot.Apps.FluepdotServer do
  ...
  def init_app(_opts) do
    ...
    # Start HTTP server
    {:ok, http_pid} = Bandit.start_link(
      plug: Flipdot.Apps.FluepdotServer.HttpHandler,
      port: @http_port,
      scheme: :http
    )
...
defmodule Flipdot.Apps.FluepdotServer.HttpHandler do
  @moduledoc """
  ...
  get "/fonts" do
    fonts = Flipdot.Font.Library.get_fonts()
    text = fonts
           |> Enum.map(fn font ->
  ...
  post "/framebuffer/text" do
    with {:ok, x, y, font_name} <- validate_text_params(conn.query_params),
      ...
      font = Flipdot.Font.Library.get_font_by_name(font_name)
      mode = Flipdot.Apps.FluepdotServer.get_rendering_mode()
      Logger.debug("Rendering text in #{mode} mode")

      # In full mode, start with empty bitmap. In differential mode, use current display
      base_bitmap = case mode do
        :full ->
      ...
      new_bitmap = Flipdot.Font.Renderer.place_text(
        base_bitmap,
        font,
        text,
        align: :left,
  ...
  put "/rendering/mode" do
    with {:ok, body, conn} <- Plug.Conn.read_body(conn),
      ...
      Flipdot.Apps.FluepdotServer.set_rendering_mode(mode)
      send_resp(conn, 200, "OK")
    else
      {:error, reason} ->
        send_resp(conn, 400, "Bad Request: #{reason}")
  ...
  defp validate_content_type(conn) do
    case get_req_header(conn, "content-type") do
      [] -> :ok  # No content type specified is OK
      ["text/plain"] -> :ok
  ...
  defp parse_rendering_mode(value), do: {:error, "Invalid rendering mode: #{value}. Use 0 for full or 1 for differential"}
end
