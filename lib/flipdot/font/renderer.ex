defmodule Flipdot.Font.Renderer do
  ...
  def place_text(target_bitmap, font, text, opts \\ []) do
    ...
    Logger.debug("Text bitmap dimensions: #{text_bitmap.width}x#{text_bitmap.height}, baseline_y: #{text_bitmap.baseline_y}")
    ...
    if Enum.empty?(text_bitmap.matrix) do
      ...
      Logger.debug("Target bitmap dimensions: #{target_bitmap.width}x#{target_bitmap.height}, baseline_y: #{target_bitmap.baseline_y}")
